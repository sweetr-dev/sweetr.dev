...
```bash
SWEETR_TAG=1.2.0 curl -fsSL https://raw.githubusercontent.com/sweetr-dev/sweetr.dev/main/bin/deploy | bash
```
...
<Steps>
  <Step title="Create a GitHub Application">
    ...
    You can use our pre-filled link to speed this up. Replace `YOUR_DOMAIN` with your actual domain:

    ```
    https://github.com/settings/apps/new?name=sweetr&url=https://YOUR_DOMAIN&webhook_active=true&webhook_url=https://YOUR_DOMAIN/api/github/callback&callback_urls[]=https://YOUR_DOMAIN/github/callback&request_oauth_on_install=true&public=false&metadata=read&pull_requests=write&statuses=write&members=read&emails=read&events[]=installation_target&events[]=organization&events[]=public&events[]=pull_request&events[]=pull_request_review&events[]=repository
    ```
    ...
    <Accordion title="Manual setup (without pre-filled link)">
      <Tabs>
        <Tab title="General">
          ...
          - Webhook
            - Check "Active"
            - Webhook URL: `https://your-domain.com/api/github/callback`
            - Generate and set a Webhook Secret
          - SSL verification
...
The Docker setup exposes the API on port 8000 and the web app on port 80.
...
If ports 80, 5432, or 6379 are already in use, either stop the conflicting service or change the port mappings in `docker-compose.yml`:

```yaml
ports:
  - "3080:80" # Map to a different host port
```
