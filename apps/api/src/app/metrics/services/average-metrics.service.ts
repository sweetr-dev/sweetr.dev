...
const buildInnerJoinClause = (workspaceId: number) => Prisma.sql`
    INNER JOIN "PullRequest" p ON pt."pullRequestId" = p."id" AND p."workspaceId" = ${workspaceId}
    INNER JOIN "GitProfile" gp ON p."authorId" = gp."id"
    INNER JOIN "TeamMember" tm ON gp."id" = tm."gitProfileId" AND tm."workspaceId" = ${workspaceId}
    INNER JOIN "WorkspaceMembership" wm ON gp."id" = wm."gitProfileId" AND wm."workspaceId" = ${workspaceId}
`;
...
export const getAverageTimeToMerge = async (filters: AverageMetricFilters) => {
  const query = Prisma.sql`
    ...
    ${buildInnerJoinClause(filters.workspaceId)}
    WHERE pt."workspaceId" = ${filters.workspaceId}
...
export const getAverageTimeForFirstReview = async (
  ...
  const query = Prisma.sql`
    ...
    ${buildInnerJoinClause(filters.workspaceId)}
    WHERE pt."workspaceId" = ${filters.workspaceId}
      AND pt."firstReviewAt" >= ${new Date(filters.startDate)} 
...
export const getAverageTimeForApproval = async (
  ...
  const query = Prisma.sql`
    ...
    ${buildInnerJoinClause(filters.workspaceId)}
    WHERE pt."workspaceId" = ${filters.workspaceId}
      AND pt."firstApprovalAt" >= ${new Date(filters.startDate)} 
