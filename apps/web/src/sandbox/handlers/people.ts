  ...
  graphql.query("PersonCodeReviews", ({ variables }) => {
    ...
    const codeReviews = pullRequestsFixture
      ...
      .map((pr, i) => ({
        ...
        id: `cr-${person.id}-${i}`,
        state: i % 3 === 0 ? CodeReviewState.CHANGES_REQUESTED : CodeReviewState.APPROVED,
        commentCount: i + 1,
        ...
        pullRequest: {
          ...
          author: {
            __typename: "Person" as const,
            id: otherPeople[i % otherPeople.length].id,
            name: otherPeople[i % otherPeople.length].name,
            handle: otherPeople[i % otherPeople.length].handle,
            avatar: otherPeople[i % otherPeople.length].avatar,
          },
